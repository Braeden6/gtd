...
class InboxRepository(SoftDeleteRepository[InboxItem]):
    ...
    async def get_by_user_id(self, user_id: UUID, processed: Optional[bool] = None) -> List[InboxItem]:
        """Get all inbox items for a user."""
        query = select(InboxItem).where(InboxItem.user_id == user_id, InboxItem.deleted_at.is_(None))
